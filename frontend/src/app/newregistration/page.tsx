...
export default function CustomerRegistrationPage() {
    ...
    return (
        <AdminPageTemplate title={view === "list" ? "Customer List" : (selectedCustomer ? "Edit Customer" : "New Customer")}>
            {view === "list" ? (
                <div className="space-y-6">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                            登録顧客数: {customers.length}
                        </p>
                        <button
                    ...
                    <div className="divide-y divide-slate-50">
                        {isLoading ? (
                            ...
                            <div className="py-20 text-center text-slate-400 text-sm italic">顧客が登録されていません</div>
                        ) : (
                            customers.map((customer) => (
                                <div key={customer.ID} className="group flex items-center justify-between py-5 px-4 hover:bg-indigo-50/50 rounded-3xl transition-colors">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400">
                                                {customer.phone} / {customer.pref_name}{customer.address1}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => { setSelectedCustomer(customer); setView("form"); }}
